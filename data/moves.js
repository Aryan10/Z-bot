...
exports.Moves = {
    ...
    beatup: {
        ...
        basePower: 0,
        basePowerCallback: function (pokemon, target, move) {
            var currentSpecies = move.allies.shift().species;
            var bp = 5 + Math.floor(currentSpecies.baseStats.atk / 10);
    ...
    charge: {
        ...
        condition: {
            ...
            onRestart: function (pokemon, source, effect) {
                if (effect && ['Electromorphosis', 'Wind Power'].includes(effect.name)) {
                    this.add('-start', pokemon, 'Charge', this.activeMove.name, '[from] ability: ' + effect.name);
                }
                else {
    ...
    crushgrip: {
        ...
        basePowerCallback: function (pokemon, target) {
            ...
            var bp = Math.floor(Math.floor((120 * (100 * Math.floor(hp * 4096 / maxHP)) + 2048 - 1) / 4096) / 100) || 1;
        ...
        onHit: function (target, source, move) {
            ...
            for (var _i = 0, _a = source.alliesAndSelf(); _i < _a.length; _i++) {
                ...
                if (pokemon.ability === target.ability)
                    continue;
                var oldAbility = pokemon.setAbility(target.ability);
                if (oldAbility) {
    ...
    doubleshock: {
        ...
        self: {
            onHit: function (pokemon) {
                pokemon.setType(pokemon.getTypes(true).map(function (type) { return type === "Electric" ? "???" : type; }));
    ...
    dragonenergy: {
        ...
        basePower: 150,
        basePowerCallback: function (pokemon, target, move) {
            var bp = move.basePower * pokemon.hp / pokemon.maxhp;
            this.debug('BP: ' + bp);
            return bp;
        },
        category: "Special",
    ...
    flail: {
        ...
        basePower: 0,
        basePowerCallback: function (pokemon, target) {
            var ratio = Math.max(Math.floor(pokemon.hp * 48 / pokemon.maxhp), 1);
            var bp;
            if (ratio < 2) {
                bp = 200;
            }
    ...
    furycutter: {
        ...
        basePowerCallback: function (pokemon, target, move) {
            ...
            var bp = this.clampIntRange(move.basePower * pokemon.volatiles['furycutter'].multiplier, 1, 160);
            this.debug('BP: ' + bp);
            return bp;
        },
        category: "Physical",
    ...
    iceball: {
        ...
        basePowerCallback: function (pokemon, target, move) {
            var bp = move.basePower;
            var iceballData = pokemon.volatiles['iceball'];
        ...
        onAfterMove: function (source, target, move) {
            var iceballData = source.volatiles["iceball"];
            if (iceballData &&
    ...
    mortalspin: {
        ...
        priority: 0,
        flags: { contact: 1, protect: 1, mirror: 1 },
        onAfterHit: function (target, pokemon, move) {
        ...
        onAfterSubDamage: function (damage, target, pokemon, move) {
            if (!move.hasSheerForce) {
                if (pokemon.hp && pokemon.removeVolatile('leechseed')) {
                    this.add('-end', pokemon, 'Leech Seed', '[from] move: Mortal Spin', '[of] ' + pokemon);
                }
    ...
    orderup: {
        ...
        onAfterMoveSecondarySelf: function (pokemon, target, move) {
            if (!pokemon.volatiles['commanded'])
                return;
            var tatsugiri = pokemon.volatiles['commanded'].source;
    ...
    pikapapow: {
        ...
        basePower: 0,
        basePowerCallback: function (pokemon) {
            var bp = Math.floor((pokemon.happiness * 10) / 25) || 1;
            this.debug('BP: ' + bp);
            return bp;
        },
        category: "Special",
    ...
    powertrip: {
        ...
        basePower: 20,
        basePowerCallback: function (pokemon, target, move) {
            var bp = move.basePower + 20 * pokemon.positiveBoosts();
            this.debug('BP: ' + bp);
            return bp;
        },
        category: "Physical",
    ...
    revivalblessing: {
        ...
        flags: {},
        onTryHit: function (source) {
            if (!source.side.pokemon.filter(function (ally) { return ally.fainted; }).length) {
                return false;
            }
        },
    ...
    rollout: {
        ...
        basePowerCallback: function (pokemon, target, move) {
            var bp = move.basePower;
            var rolloutData = pokemon.volatiles['rollout'];
        ...
        onAfterMove: function (source, target, move) {
            var rolloutData = source.volatiles["rollout"];
            if (rolloutData &&
    ...
    safeguard: {
        ...
        condition: {
            ...
            onSideStart: function (side, source) {
                if (source === null || source === void 0 ? void 0 : source.hasAbility('persistent')) {
    ...
    skillswap: {
        ...
        onTryHit: function (target, source) {
            ...
            var sourceCanBeSet = this.runEvent('SetAbility', source, source, this.effect, targetAbility);
            if (!sourceCanBeSet)
                return sourceCanBeSet;
            var targetCanBeSet = this.runEvent('SetAbility', target, source, this.effect, sourceAbility);
            if (!targetCanBeSet)
    ...
    solarbeam: {
        ...
        onBasePower: function (basePower, pokemon, target) {
            var weakWeathers = ['raindance', 'primordialsea', 'sandstorm', 'hail', 'snow'];
    ...
    takeheart: {
        ...
        flags: { snatch: 1 },
        onHit: function (pokemon) {
            var success = !!this.boost({ spa: 1, spd: 1 });
            return pokemon.cureStatus() || success;
        },
        secondary: null,
        target: "self",
    ...
    tidyup: {
        ...
        onHit: function (pokemon) {
            ...
            var removeAll = ['spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge'];
            var sides = __spreadArray([pokemon.side], pokemon.side.foeSidesWithConditions(), true);
            for (var _b = 0, sides_2 = sides; _b < sides_2.length; _b++) {
                var side = sides_2[_b];
                for (var _c = 0, removeAll_3 = removeAll; _c < removeAll_3.length; _c++) {
                    var sideCondition = removeAll_3[_c];
